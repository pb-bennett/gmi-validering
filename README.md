...
- **Minimal anonymous usage tracking.** The only data persisted server-side is aggregated **location** data for both (1) the dataset kommune (derived from file coordinates via the [Geonorge API](https://ws.geonorge.no/)) and (2) the uploader’s rough location (from request geo). This is stored as aggregate counters in Supabase and is used solely to understand geographic adoption. No IP addresses, filenames, or file contents are stored.
