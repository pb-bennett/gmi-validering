...
- **Minimal anonymous usage tracking.** The only data persisted server-side is the **kommune** (municipality) where the uploaded data originates, derived from the coordinates in the file via the [Geonorge API](https://ws.geonorge.no/). This is stored as an aggregate counter (municipality + date) in Supabase and is used solely to understand geographic adoption. No IP addresses, filenames, or file contents are stored.
