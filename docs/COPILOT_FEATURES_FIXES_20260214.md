...
5. I need to deomstrate this app to a group of other engineers who work with this type of data in other municipalities in my county this coming week. I feel the app is fairly well filled out now and very functional. I am considering adding a button to open a modal that displays a large QR code so the people at the meeting can easily get the URL if they want to try the app. The app is not mobile friendly but they can at least save the URL. What do you think?
   I would also like to give people the github repo link. the repo is public. Maybe stupid to have 2 QR codes on the same page, so maybe the modal should have 2 tabs, one for each. default being the URL for the page. What do you think?
   Logos in centers of the QR codes. Can we have the github logo for the github QR code and a suitable icon from the library for the main URL. something suitable for the app`COMPLETE`
