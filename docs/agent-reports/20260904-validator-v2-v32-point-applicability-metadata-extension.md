...
| `LOK` | `APPLICABLE` | `UNKNOWN` | `UNKNOWN` | `APPLICABLE` |
...
Totals are **71 `APPLICABLE`**, **7 `NOT_APPLICABLE`**, and **10 `UNKNOWN`** cells, 88 total. The four `KMR` cells and four `SUMP` cells are explicit reviewed-but-unresolved canonical `UNKNOWN` cells, not inferred `NOT_APPLICABLE` cells. The two `LOK` unresolved cells are also explicit canonical `UNKNOWN` cells.

## Policy semantics and provenance

`APPLICABLE` states that the field belongs to the exact Tema semantic domain; it does not mean required. `NOT_APPLICABLE` is used only for the seven explicit positive PROJECT/DOMAIN POLICY decisions for STR and KRN. It is never generated as the complement of `APPLICABLE`. `UNKNOWN` means that no positive applicability or non-applicability decision has been established.
...
`getPointFieldApplicability` remains an exact Tema plus canonical-field lookup. It performs no case folding, whitespace trimming, alias resolution, prefix or substring matching, or inference from Type, geometry, field presence, prevalence, neighboring objects, historical population, or target values. Legacy `KUMi` is not an alias for current `KUMI`. Unsupported, unlisted, current-but-unapproved, and unsupported-field combinations return a newly allocated `UNKNOWN` default without changing policy state. Explicit `LOK`, `KMR`, and `SUMP` UNKNOWN combinations return their canonical explicit policy cells.
...
- Focused test: `node --test tests/validationV2PointFieldApplicability.test.mjs` — 9/9 passed.
- Full repository test suite: `node --test` — 295/295 passed.
