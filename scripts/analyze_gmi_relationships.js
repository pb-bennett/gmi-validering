...
const TARGET_FIELDS = [
    { key: 'Bredde', aliases: ['Bredde', 'Bredde (diameter)', 'DIMENSJON', 'DIM', 'BREDDE'] },
        ...
        if (line.startsWith('[P_]')) {
            ...
            if (fcode) {
                ...
                TARGET_FIELDS.forEach(field => {
                    feature[field.key] = getFieldValue(names, values, field.aliases);
                });
...
function analyze() {
    const files = fs.readdirSync(GMI_DIR).filter(f => f.endsWith('.gmi'));
    let allFeatures = [];
    ...
    for (const feature of allFeatures) {
        ...
        TARGET_FIELDS.forEach(field => {
            ...
            if (val) {
                ...
                stats[fcode].fields[field.key].values[valStr] = (stats[fcode].fields[field.key].values[valStr] || 0) + 1;
            }
        });
    }

    // Generate Markdown
    let md = '# GMI Analysis: Relationship between S_FCODE and Attributes\n\n';
    ...
    for (const fcode of sortedFcodes) {
        ...
        TARGET_FIELDS.forEach(field => {
            ...
            const percent = ((fieldStats.present / fcodeStats.count) * 100).toFixed(1);
            ...
            md += `| ${field.key} | ${fieldStats.present} | ${percent}% | ${sortedValues || '-'} |\n`;
        });
        md += '\n';
    }
