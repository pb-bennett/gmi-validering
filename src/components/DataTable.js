...
export default function DataTable() {
  ...
  const [activeTab, setActiveTab] = useState('punkter'); // 'punkter' or 'ledninger'
  ...
  useEffect(() => {
    ...
    // Compute and log header/button positions for debugging
    const update = () => {
      const btn = document.getElementById(
        'datatable-close-button-inline'
      );
      const header = document.getElementById('datatable-header');

      if (header) {
        const hr = header.getBoundingClientRect();
        console.log('DEBUG: header rect', hr);

        // compute position for fixed close button: top-right corner inset 8px
        const top = Math.max(hr.top + 8, 8);
        const left = Math.max(hr.right - 44, 8);
        setClosePos({ top, left });
      }

      if (btn) {
        console.log('DEBUG: inline button exists', btn);
        console.log(
          'DEBUG: inline button computed style',
          getComputedStyle(btn)
        );
        console.log(
          'DEBUG: inline button rect',
          btn.getBoundingClientRect()
        );
        if (btn.parentElement)
          console.log(
            'DEBUG: inline button parent computed style',
            getComputedStyle(btn.parentElement)
          );
      } else {
        console.log('DEBUG: inline button NOT found in DOM');
      }
    };
    ...
    const t = setInterval(update, 500); // keep updating briefly in case of layout shifts
  ...
  // Create a portal-style close button attached to document.body to guarantee visibility
  useEffect(() => {
    const headerEl = document.getElementById('datatable-header');
    if (!headerEl || !document.body) return;

    const btn = document.createElement('button');
    btn.id = 'datatable-portal-close';
    btn.innerText = '×';
    btn.setAttribute('aria-label', 'Lukk tabell');
    ...
    const positionBtn = () => {
      ...
      // Place at top-right inside viewport, but aligned to header right
      const top = Math.max(8, rect.top + 8);
      const left = Math.max(
        8,
        Math.min(window.innerWidth - 48, rect.right - 48)
      );
    ...
    btn.addEventListener('click', () => {
      console.log('Portal close clicked');
      toggleDataTable();
    });
    ...
    // In case of layout shifts
    const interval = setInterval(positionBtn, 500);
