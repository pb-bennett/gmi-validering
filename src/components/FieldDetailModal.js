  ...
  const sortedValues = field.stats.valueCounts 
    ? Object.entries(field.stats.valueCounts)
        .map(([value, count]) => ({
          ...
          isValid: !field.acceptableValues || field.acceptableValues.length === 0 || 
                   field.acceptableValues.some(av => String(av.value) === String(value))
        }))
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {activeTab === 'overview' ? (
          <>
            {/* Description */}
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Beskrivelse</h3>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                <div className="text-xs text-gray-500 mb-1">Utfyllingsgrad</div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                  <div 
                    className={`h-1.5 rounded-full ${
                      field.stats.completion === 100 ? 'bg-green-500' : 
                      field.stats.completion > 50 ? 'bg-yellow-500' : 'bg-red-500'
                    }`}
              ...
              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div className={`text-lg font-bold ${
                  ...
                  field.status === 'warning' ? 'text-yellow-600' : 'text-green-600'
                }`}>
                  {field.status === 'error' ? 'Mangler data' :
                   field.status === 'warning' ? 'Delvis utfylt' : 'OK'}
                </div>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100 col-span-2">
                 ...
                 <div className="flex gap-2 flex-wrap">
                    ...
                    {field.objectTypes.map(type => (
                      <span key={type} className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 capitalize">
                        {type}
                      </span>
                    ))}
                  </div>
            ...
            {/* Detailed Stats */}
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Statistikk</h3>
              <div className="bg-white border rounded-lg divide-y text-sm">
                <div className="flex justify-between p-2">
                  <span className="text-gray-600">Totalt antall objekter</span>
                  <span className="font-mono font-medium">{field.stats.total}</span>
                </div>
                <div className="flex justify-between p-2">
                  <span className="text-gray-600">Utfylt</span>
                  <span className="font-mono font-medium text-green-600">{field.stats.present}</span>
                </div>
                <div className="flex justify-between p-2">
                  <span className="text-gray-600">Mangler</span>
                  <span className="font-mono font-medium text-red-600">{field.stats.missing}</span>
                </div>
                {field.stats.unexpected > 0 && (
                  <div className="flex justify-between p-2">
                    <span className="text-gray-600">Uventet (feil type)</span>
                    <span className="font-mono font-medium text-orange-600">{field.stats.unexpected}</span>
                  </div>
                )}
                {field.acceptableValues && (
                  <div className="flex justify-between p-2">
                    <span className="text-gray-600">Ugyldige verdier</span>
                    <span className="font-mono font-medium text-yellow-600">{field.stats.invalid}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Acceptable Values */}
            {field.acceptableValues && field.acceptableValues.length > 0 && (
              <div>
                  ...
                  <div className="overflow-y-auto max-h-[400px]">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr>
                          <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-24">Kode</th>
                          <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Beskrivelse</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {field.acceptableValues.map((val) => (
                          <tr key={val.value} className="hover:bg-gray-50">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {sortedValues.map((item) => (
                      <tr key={item.value} className={!item.isValid ? 'bg-red-50' : ''}>
