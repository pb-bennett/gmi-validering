...
export default function MissingFieldsReport({ onClose }) {
  const setFilteredFeatureIds = useStore((state) => state.setFilteredFeatureIds);
  ...
  const reportData = useMemo(() => {
    ...
    validationResults.forEach((field) => {
      ...
      const item = {
        ...
        invalidText: field.stats.invalid > 0 ? `${field.stats.invalid} ugyldige` : null,
      };
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="p-4 border-b flex justify-between items-center bg-gray-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900">Mangelliste</h2>
            <p className="text-xs text-gray-500">Oversikt over manglende eller ugyldige feltverdier</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 border-b border-gray-200">
          ...
          <div className="space-y-2">
            {reportData.ledninger.length === 0 ? (
              <p className="text-xs text-gray-500 italic">Ingen mangler funnet på ledninger.</p>
            ) : (
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="space-y-2">
            {reportData.punkter.length === 0 ? (
              <p className="text-xs text-gray-500 italic">Ingen mangler funnet på punkter.</p>
            ) : (
...
function ReportItem({ field, onClick }) {
  const isError = field.status === 'error';
  
  return (
    <div 
      onClick={onClick}
      className={`p-3 rounded border cursor-pointer transition-all hover:shadow-sm ${isError ? 'border-red-200 bg-red-50 hover:bg-red-100' : 'border-yellow-200 bg-yellow-50 hover:bg-yellow-100'}`}
    >
      <div className="flex justify-between items-start mb-1">
        <div>
          <h4 className="font-semibold text-sm text-gray-900">{field.label || field.fieldKey}</h4>
        ...
        <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded uppercase ${isError ? 'bg-red-200 text-red-800' : 'bg-yellow-200 text-yellow-800'}`}>
      ...
      <div className="text-xs text-gray-700 space-y-0.5">
        <div className="flex justify-between">
          <span>Mangler:</span>
          <span className="font-mono font-medium">{field.stats.missing} / {field.stats.total}</span>
        </div>
        {field.stats.invalid > 0 && (
          <div className="flex justify-between text-red-600">
            <span>Ugyldige:</span>
            <span className="font-mono font-medium">{field.stats.invalid}</span>
          </div>
        )}
        {field.stats.unexpected > 0 && (
          <div className="flex justify-between text-yellow-600">
            <span>Uventede:</span>
            <span className="font-mono font-medium">{field.stats.unexpected}</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-2 h-1.5 bg-gray-200 rounded-full overflow-hidden">
        <div 
          className={`h-full ${isError ? 'bg-red-500' : 'bg-yellow-500'}`} 
