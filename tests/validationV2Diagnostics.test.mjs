...
test('bounded evidence is explicitly non-exhaustive and sensitive evidence stays absent', () => {
  ...
  const sensitiveRule = { ...rule, canonicalFieldId: 'caseNumber' };
  const sensitiveFinding = { ...finding('CHECK', 'UNUSUAL_VALID_VALUE', 0, 'PRIVATE'), rule: sensitiveRule, canonicalFieldId: 'caseNumber' };
  const sensitiveModel = buildFieldDiagnostics({ result: { ruleResults: [{ rule: sensitiveRule, geometryBreakdown: { point: { evaluatedCount: 1, failCount: 0, checkCount: 1, indeterminateCount: 0, passCount: 0 } }, findings: [sensitiveFinding] }], outcomes: [], schemaFindings: [] }, rule: sensitiveRule, field: { canonicalFieldId: 'caseNumber', displayName: 'Saksnummer' }, geometryScope: 'point', summary: { objectCount: 1 } });
  assert.deepEqual(sensitiveModel.diagnostics[0].values, []);
  assert.doesNotMatch(renderValidationV2Diagnostic(sensitiveModel.diagnostics[0]), /PRIVATE/);
});
