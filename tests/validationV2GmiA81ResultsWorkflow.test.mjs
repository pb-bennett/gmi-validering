...
test('all active rules resolve exact canonical short labels', () => {
  const expected = [
    'Adkomst', 'Eier', 'Utvendig_høyde', 'NOBB/VAVVS-nummer', 'NOBB/VAVVS-nummer ramme',
    ...
    'Tema', 'Type', 'Innvendig/utvendig', 'Bredde', 'Lengde', 'Tykkelse', 'Dimensjon', 'Nett-type', 'Rørform',
  ];
  const rules = getValidationRules();
  const labels = [...new Set(rules.map((rule) => getFieldInformation(rule.canonicalFieldId).displayName))];
  assert.deepEqual([...labels].sort(), [...expected, 'Material', 'Kumform', 'Byggemetode', 'Kjegle', 'Avst_BunnInnvUnderUtv', 'Merknad'].sort());
...
test('point and line presentation universes are the reviewed active counts', () => {
  ...
  assert.equal(getValidationV2PresentationRules(results, 'point').length, 38);
